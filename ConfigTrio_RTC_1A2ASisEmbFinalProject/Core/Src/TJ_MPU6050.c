...
//2- i2c Read
void I2C_Read(uint8_t ADDR, uint8_t *i2cBif, uint8_t NofData, int r)
{
	uint8_t i2cBuf[2];
	uint8_t MPUADDR;
	...
	if(r==1){
		...
		/*RIGA NOSTRA*/
		HAL_I2C_Master_Transmit(&i2cHandler, (0x69<<1), i2cBuf, 1, 10);
		HAL_I2C_Master_Receive(&i2cHandler, (0x69<<1), i2cBif, NofData, 100);
		/*FINE RIGA NOSTRA*/
	}
...
void I2C_Write8(uint8_t ADDR, uint8_t data)
{
	...
	/*RIGA NOSTRA*/
	HAL_I2C_Master_Transmit(&i2cHandler, (0x69<<1), i2cData, 2,100);
	/*FINE RIGA NOSTRA*/
}
...
uint8_t MPU6050_Get_SMPRT_DIV(void)
{
	...
	I2C_Read(SMPLRT_DIV_REG, &Buffer, 1,0);
	return Buffer;
}
...
uint8_t MPU6050_Get_FSYNC(void)
{
	...
	I2C_Read(CONFIG_REG, &Buffer, 1,0);
	Buffer &= 0x38;
	return (Buffer>>3);
}
...
void MPU6050_Get_Accel_RawData(RawData_Def *rawDef, int r)
{
	uint8_t i2cBuf[2];
	uint8_t AcceArr[6], GyroArr[6];

	I2C_Read(INT_STATUS_REG, &i2cBuf[1],1,r);
	if((i2cBuf[1]&&0x01))
	{
		I2C_Read(ACCEL_XOUT_H_REG, AcceArr,6,r);
		...
		//Gyro Raw Data
		I2C_Read(GYRO_XOUT_H_REG, GyroArr,6,r);
...
void MPU6050_Get_Accel_Scale(ScaledData_Def *scaledDef)
{
	...
	MPU6050_Get_Accel_RawData(&AccelRData, 1);
